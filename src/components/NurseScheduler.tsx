...
export function NurseScheduler({ isAdmin, currentUser }: NurseSchedulerProps) {
  ...
  const { isConnected, isChecking, error: connectionError } = useSupabaseConnection();
  ...
  const handleUpdateSchedule = async (nurseId: string, day: number, status: ShiftStatus, notes: Note[]) => {
    if (!isAdmin && currentUser.id !== nurseId) {
      return;
    }

    const year = currentDate.getFullYear();
    const month = currentDate.getMonth() + 1;
    const currentSchedule = getCurrentSchedule(nurseId);
    const updatedSchedule = [...currentSchedule];
    updatedSchedule[day] = { status, notes };

    try {
      await updateSchedule(nurseId, year, month, updatedSchedule);
    } catch (error) {
  ...
  const getCurrentSchedule = (nurseId: string) => {
    const scheduleKey = `${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${nurseId}`;
    return schedule[scheduleKey] || Array(getDaysInMonth(currentDate)).fill({ status: 'none', notes: [] });
  };
  ...
  if (!isConnected || connectionError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
        ...
        <h2 className="text-xl font-semibold text-gray-800 mb-2">Problème de connexion</h2>
